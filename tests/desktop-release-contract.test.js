...
// Windows metadata, Electron runtime identity, window titles and tray identity
// must all present one product name.
for (const config of [manifest.build, clientConfig, fullPortableConfig]) {
...
// The only server desktop artifact is the full offline portable executable.
// It embeds the verified Windows and Android downloads under resources/offline-downloads.
...
for (const value of [...mainFiles, ...mainUnpacked, ...mainResources]) {
  assert.doesNotMatch(value, /(^|[\\/])mac(?:[\\/]|$)|SyncWatch同步观影-Client-v2\.1\.7\.exe/i,
    `main desktop package retains a removed or obsolete payload: ${value}`);
}
...
for (const [label, config] of [['Full portable', fullPortableConfig], ['Default full portable', manifest.build]]) {
...
assert.equal(manifest.build.portable.artifactName, `SyncWatch-v${manifest.version}-Full-Offline-Portable-\${arch}.exe`);
assert.equal(fs.existsSync(path.join(root, 'electron-builder-windows-installer.json')), false,
  'removed Full Offline installer configuration must not return');
...
assert.doesNotMatch(electronServer, /offline-downloads['"], ['"]mac/);

assert.doesNotMatch(windowsBuild, /SyncWatch同步观影-Client-v2\.1\.7\.exe/);
assert.match(windowsBuild, /SyncWatch-Experience-Client-Portable-v\$\(\$manifest\.version\)-x64\.exe/);
...
assert.match(atomicReleaseWorkflow, /find dist -maxdepth 1 -type f[\s\S]*= \"10\"/);
