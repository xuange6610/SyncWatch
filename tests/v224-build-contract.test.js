...
for (const config of builders) {
  ...
  assert.doesNotMatch(serialized, /v?2\.2\.3/);
}
...
assert.match(androidGradle, /versionCode\s+20204/);
assert.match(androidGradle, /versionName\s+['"]2\.2\.4['"]/);
assert.match(read('mobile/app/src/main/java/com/xuan/syncwatch/MainActivity.java'), /SyncWatchAndroid\/v2\.2\.4/);
assert.match(read('mobile/app/src/main/java/com/xuan/syncwatch/MobileServerService.java'), /SyncWatch同步观影-v2\.2\.4\.apk/);
...
for (const relative of [
  ...
  'server/macos-distribution.js', 'mac-distribution.example.json', 'mobile/build-apk.ps1'
]) {
  const source = read(relative);
  assert.doesNotMatch(source, /v?2\.2\.3|20203/, `${relative} still contains the previous release version`);
}
...
for (const required of [
  ...
  'tests/v224-build-contract.test.js'
...
console.log('v2.2.4 non-document version and build closure contracts passed.');
