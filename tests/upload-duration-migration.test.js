...
async function main() {
  ...
  // A previous v2.3.9 candidate could have persisted the former five-minute
  // default together with the configured flag. Upgrading must clear it.
  fs.writeFileSync(path.join(dataDir, 'config.json'), JSON.stringify({
    version: 13,
    admin: { uploadVideoDurationLimitSeconds: 300, uploadVideoDurationLimitConfigured: true }
  }));
  ...
  try {
    ...
    assert.equal(migrated.admin.uploadVideoDurationLimitPolicyVersion, 0);
    await server.close();
    ...
    migrated.admin.uploadVideoDurationLimitPolicyVersion = 1;
