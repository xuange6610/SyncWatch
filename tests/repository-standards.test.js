...
const candidateReleaseBanner = readme.includes('v2.3.0') && /候选(?:分支|发布|版本)|发布准备|pending|待发布/.test(readme);
...
assert.ok(new RegExp(`${escapeRegExp(latestStableTag)}.*发布并设为 Latest`, 's').test(readme) || /v2\.2\.9.*候选(?:分支|发布|版本).*验证|候选发布仍按此清单验证|发布准备|pending/s.test(readme));
...
assert.ok(agents.includes(stableAgentSnapshot) || (agents.includes(`${latestStableTag}`) && (agents.includes('候选') || agents.includes('发布准备'))));
...
assert.ok(maintainerRequirements.includes(stableMaintainerSnapshot) || (maintainerRequirements.includes(`${latestStableTag}`) && (maintainerRequirements.includes('候选') || maintainerRequirements.includes('发布准备'))));
...
assert.match(releaseManifest, new RegExp(`(?:${escapeRegExp(latestStableTag)}.*Latest|${escapeRegExp(latestStableTag)}.*(?:当前只是候选|发布准备))`, 's'));
