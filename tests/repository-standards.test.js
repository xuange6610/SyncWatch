...
assert.ok(maintainerRequirements.includes(stableMaintainerSnapshot) || maintainerRequirements.includes('当前版本：`v2.2.6` 候选修正版') || (maintainerRequirements.includes('当前线上正式版本：`v2.2.5`') && maintainerRequirements.includes(`${latestStableTag}`) && maintainerRequirements.includes('仍是候选')));
const releaseManifest = read('docs/release/release-manifest.md');
assert.match(releaseManifest, /Source code \(zip\)/);
assert.match(releaseManifest, /10 个维护者资产/);
