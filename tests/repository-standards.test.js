...
const candidateReleaseBanner = readme.includes('当前正式发布：[v2.2.4](https://github.com/xuange6610/SyncWatch/releases/tag/v2.2.4)') && readme.includes('v2.2.5') && readme.includes('候选分支');
...
assert.ok(agents.includes(stableAgentSnapshot) || (agents.includes('当前线上正式版本仍为 `v2.2.4`') && agents.includes(`${latestStableTag}`) && agents.includes('候选分支')));
...
assert.ok(maintainerRequirements.includes(stableMaintainerSnapshot) || (maintainerRequirements.includes('当前线上正式版本：`v2.2.4`') && maintainerRequirements.includes(`${latestStableTag}`) && maintainerRequirements.includes('仍是候选')));
