...
assert.match(readme, new RegExp(`当前正式发布：\\[${escapeRegExp(latestStableTag)}\\]\\(https:\\/\\/github\\.com\\/xuange6610\\/SyncWatch\\/releases\\/tag\\/${escapeRegExp(latestStableTag)}\\).*Latest`));
assert.match(readme, new RegExp(`${escapeRegExp(latestStableTag)}.*发布并设为 Latest`, 's'));
