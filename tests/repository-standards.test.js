...
assert.match(agents, new RegExp(`当前最新正式版本为 \`${escapeRegExp(latestStableTag)}\``));
...
assert.match(maintainerRequirements, new RegExp(`当前线上正式版本：\`${escapeRegExp(latestStableTag)}\``));
...
assert.match(releaseManifest, new RegExp(`${escapeRegExp(latestStableTag)}.*Latest`, 's'));
