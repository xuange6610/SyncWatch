...
- 所有公开版本、分支、Tag、Release 标题、下载链接和公告使用 `vX.Y.Z` 前缀；`package.json` 和 Android 工具链字段保留纯数字 SemVer。当前最新正式版本为 `v2.2.4`，后续版本必须在真实升级后再递增。
...
- 当前稳定分支是 `main`，2026-08-27 复核时远端 `main` 与 annotated Tag `v2.2.4` 指向提交 `66eabf7a6129e51b33206f9971d56172e5614c0f`。`v2.2.4` Release 已设为 Latest，API 有 26 个维护者资产，页面连同 GitHub 源码归档可见文件为 28 个；原子工作流 run `33040168896` attempt 3 已完成远端哈希回读。
