...
- 所有公开版本、分支、Tag、Release 标题、下载链接和公告使用 `vX.Y.Z` 前缀；`package.json` 和 Android 工具链字段保留纯数字 SemVer。当前线上正式版本仍为 `v2.2.4`；`v2.2.5` 仅在 `release/v2.2.5` 候选分支，必须完成真实构建、哈希回读和 Release API 验证后才能切换为正式版。
...
- 当前稳定分支仍是 `main`，远端正式基线为 annotated Tag `v2.2.4`，提交 `66eabf7a6129e51b33206f9971d56172e5614c0f`；`v2.2.4` Release 保持 Latest、26 个维护者资产和 28 个可见文件。`release/v2.2.5` 当前候选提交为 `f96be3d`（尚未创建 tag、Release 或 Actions 回执）；发布完成后必须用实际 commit SHA、run ID、API 资产数和页面核验结果回填本节。
