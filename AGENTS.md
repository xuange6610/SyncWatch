...
当前最新正式版本为 `v2.4.2`；`v2.4.3` 正在准备 NAT/反向代理媒体响应修复，尚未完成最终构建与 Release 验收；历史版本资产保持不变。

当前稳定分支为 `main`，正式源码版本为 `v2.4.2`；v2.4.2 已完成真实构建、启动、哈希和 Release 资产验收。`release/v2.4.3` 仅在全部证据完成后才能合并和发布。历史 Release、历史 Tag 和历史资产保持不变。

当前工作区包含已发布的 `v2.4.2` 修复：媒体格式、移动端片库、聊天、成员 UI 回归及最终 Tag 成品验收均已完成。
