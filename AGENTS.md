...
当前最新正式版本为 `v2.4.0`（候选，待最终构建与 Release 验收）；源码、Windows/Android 构建、远端资产和页面验收完成后方可公开，历史版本资产保持不变。

当前稳定分支为 `main`，正式源码版本为 `v2.3.7`；v2.3.7 已完成真实构建、启动、哈希和 Release 资产验收。历史 Release、历史 Tag 和历史资产保持不变。

当前工作区包含 `v2.3.9` 候选修复：发布前必须完成登录音乐与 Android 移动端 UI 回归，并从最终 Tag 重新构建和核验资产。
