...
当前最新正式版本为 `v2.4.3`；v2.4.4 修复已合并到 `main`，正在进入最终 Tag、原子构建和 Release 验收；历史版本资产保持不变。

当前稳定分支为 `main`，正式源码版本为 `v2.4.3`；v2.4.4 源码已完成合并，最终 Tag 对应的真实构建、启动、哈希和 Release 资产验收仍按发布工作流执行。历史 Release、历史 Tag 和历史资产保持不变。

当前工作区包含已发布的 `v2.4.3` 修复：媒体格式、NAT/反向代理 Range 响应、移动端片库、聊天、成员 UI 回归及最终 Tag 成品验收均已完成。

当前工作区另有待发布的 `v2.4.4` 修复：服务端播放时钟按媒体时长封顶，客户端在原片不可解码且兼容版已就绪时自动选择 H.264/AAC 源。最终 Tag 必须重新完成 Windows/Android 构建、启动、哈希和 Release 页面验收后，才能成为线上 Latest。
