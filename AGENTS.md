...
- 所有公开版本、分支、Tag、Release 标题、下载链接和公告使用 `vX.Y.Z` 前缀；`package.json` 和 Android 工具链字段保留纯数字 SemVer。当前最新正式版本为 `v2.2.3`，`v2.2.4` 仍是待完成全部发布门禁的源码候选，后续版本必须在真实升级后再递增。
...
- 当前稳定分支是 `main`，2026-08-27 复核时远端 `main` 与 `v2.2.3` 指向提交 `51f8c6941614b6c7a1e31800df62115018d2468f`。当前最新正式版本仍是 `v2.2.3`；该 Release 已设为 Latest，API 有 26 个维护者资产，页面可见文件为 28 个。`release/v2.2.4` 只有完成最终 PR、annotated Tag、原子工作流、26+2 资产和远端哈希回读后才能改成正式状态。
