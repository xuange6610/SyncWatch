...
当前最新正式版本为 `v2.3.5`；源码、Windows/Android 构建、远端资产和页面验收已完成，后续修复仍须遵守同版本资产与历史版本保留规则。

当前稳定分支为 `main`，源码版本为 `v2.3.5`；v2.3.5 已完成真实构建、启动、哈希和 Release 资产验收。历史 Release、历史 Tag 和历史资产保持不变。

当前工作区包含 `v2.3.6` 候选修复：发布前必须完成旧数据目录启动兼容、固定端口冲突提示、在线成员双击事件隔离和 Android/Windows 全量回归，并从最终 Tag 重新构建和核验资产。
