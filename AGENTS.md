...
当前最新正式版本为 `v2.3.2`；源码、Windows/Android 构建、远端资产和页面验收已完成，后续修复仍须遵守同版本资产与历史版本保留规则。

当前稳定分支为 `main`，源码版本为 `v2.3.2`；v2.3.2 已完成真实构建、启动、哈希和 Release 资产验收。历史 Release、历史 Tag 和历史资产保持不变。

当前工作区包含 `v2.3.4` 候选修复：发布前必须完成 Android 手机网页布局、全屏方向行为、管理中心、共享链路回归，并从最终 Tag 重新构建和核验资产。
