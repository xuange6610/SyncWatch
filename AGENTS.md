...
当前最新正式版本为 `v2.3.1`；源码、Windows/Android 构建、远端资产和页面验收已完成，后续修复仍须遵守同版本资产与历史版本保留规则。

当前稳定分支为 `main`，源码版本为 `v2.3.1`；本轮共享性能、音源状态、主题回执、Android 启动迁移和桌面启动重试已完成构建、启动与远端资产验收。v2.3.0 历史 Release、历史 Tag 和历史资产保持不变。

当前工作区包含 `v2.3.2` 候选修复：发布前必须完成移动端布局、快捷键、管理中心、上传限制、封面与共享链路回归，并从最终 Tag 重新构建和核验资产。
