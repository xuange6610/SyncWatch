...
当前最新正式版本为 `v2.4.3`；已完成 NAT/反向代理媒体响应修复、源码门禁、Windows/Android 构建、启动验收、哈希回读和公开 Release；历史版本资产保持不变。

当前稳定分支为 `main`，正式源码版本为 `v2.4.3`；v2.4.3 已完成真实构建、启动、哈希和 Release 资产验收。历史 Release、历史 Tag 和历史资产保持不变。

当前工作区包含已发布的 `v2.4.3` 修复：媒体格式、NAT/反向代理 Range 响应、移动端片库、聊天、成员 UI 回归及最终 Tag 成品验收均已完成。

当前工作区另有待发布的 `v2.4.4` 候选修复：服务端播放时钟按媒体时长封顶，客户端在原片不可解码且兼容版已就绪时自动选择 H.264/AAC 源。Windows/Android 本地成品已完成构建、哈希与 Windows 核心流程验收；最终 Tag、Release、Android 模拟器启动和云服务器部署仍待完成，不得当作线上 Latest。
...
- `v2.3.0` 运行 `33365811610` 曾失败于 Windows EPIPE 的 `app:browser-window-created`；后续修复与 Android ADB 重试已由最终运行 `33370280271` 全部验证，v2.3.0 现为公开 Latest。失败运行仍只作历史诊断证据。
...
- v2.3.6 原子运行 `33589690484` 的源码、官方资产、Android 签名 APK 和 Windows 基础包均成功；Android 模拟器在设备已启动后由 `reactivecircus/android-emulator-runner` 内部 `adb shell input keyevent 82` 收到 `Failure calling service input: Broken pipe (32)`，仓库 smoke 尚未执行，未替换 Release 资产。修复为在 Action 的 pre-launch 阶段安装仅针对该解锁命令的 ADB 重试包装，并关闭非必要动画设置；仓库 smoke 继续执行严格的 ADB/package/启动检查。修复后必须从更新后的最终 v2.3.6 Tag 重新执行一次完整原子发布。
- 重跑 `33591670539` 因 ADB 包装器首次实现把真实二进制路径错误地按仓库脚本目录解析，导致模拟器 job 长时间无结果；运行已取消，未生成或上传正式 Release 资产。包装器现固定读取 `$ANDROID_HOME/platform-tools/adb.syncwatch-real`，仅代理 Action 的解锁命令，其余命令透传；后续重跑前必须通过源码契约并确认该绝对路径。
- 最终重跑 `33593663723` 已成功启动模拟器、安装 APK、恢复 `com.xuan.syncwatch/.MainActivity` 并上传启动证据，但 job 仍失败于 pre-launch 脚本的 `/bin/sh: set: Illegal option -o pipefail`；该步骤失败后 Action 继续运行，故不是应用故障。修复为移除 pre-launch 中 Bash 专属 `set -euo pipefail`，并增加契约防止再次使用；修复后必须从更新后的最终 v2.3.6 Tag 重新执行一次完整原子发布。
- 最终重跑 `33596284469` 暴露第三方 Action 会按换行拆分 `pre-emulator-launch-script`，多行 `if ... fi` 被拆成独立 `/bin/sh -c` 命令而报 `Syntax error: end of file unexpected (expecting "fi")`；模拟器、APK 安装、主 Activity 和启动证据均成功，Release 汇总因该步骤失败被跳过。修复为调用独立的 `scripts/android-emulator-prepare.sh`（pre-launch 仅保留单行命令），并增加契约锁定；修复后必须从更新后的最终 v2.3.6 Tag 重新执行一次完整原子发布。
...
- 当前最新正式版本为 `v2.3.0`；最终注释 Tag 对象为 `d5db7eff01faa57624b7d750f161eba982fb0d0c`，指向提交 `f931bf2097c03712d90d3aa7c30314c675e8d5e7` 和源码树 `d6f5ad6794d2ba119fad539cc52686b8df132b95`；Actions run `33405585536` 已完成 8 个维护者资产、2 个 GitHub 源码归档、Windows/Android 启动、Microsoft Defender、Latest 和远端哈希核对。v2.2.9 及更早版本的交付问题仅作历史复盘。
- `main`、`release/v2.3.0` 和最终 Tag 已对齐；共享性能、音源状态、主题回执和桌面启动重试已进入当前 Release，不再是待覆盖源码。
...
### 34. v2.3.0 最终原子发布完成（2026-08-31）

- 最终注释 Tag 对象 `11203ae2587029415b7332b9409b22ac0f64bdaf` 指向提交 `c2086ca6fa1a64a4ffa117361ae42921b3ab4956`；原子运行 `33370280271` 成功完成源码、官方文件、Android、Windows、Defender、10 文件审计、上传和远端 SHA-256 回读。
- Release API 有 8 个维护者资产，页面另有 2 个源码归档；v2.3.0 已公开为 Latest。此前 EPIPE 与 ADB 失败运行只保留为历史复盘，不再代表当前状态。
- 首次发布后的工作区改动当时尚未进入资产。用户随后明确授权执行 v2.3.0 同版本纠正覆盖；只能通过原子工作流先验证新 8 项、再替换并删除该版本旧 8 项，禁止提前清空公开资产或影响历史 Release。

### 35. v2.3.0 同版本纠正覆盖授权（2026-08-31）

- 本轮覆盖范围包括：更低延迟的画面/音频共享、原生分辨率与设备最高刷新率默认请求、房间实时音源标题/进程状态及停止清理、主题同步接受/拒绝/已应用回执、Electron 启动等待窗口和本机页面加载重试。
- 用户明确要求覆盖更新并删除之前的 v2.3.0 旧资产；删除范围只限安全切换成功后被新 8 项取代的当前版本旧资产。所有历史 Release、历史 Tag、历史资产和备份继续保留。
- 原子覆盖成功前，运行 `33370280271`、Tag 对象 `11203ae2587029415b7332b9409b22ac0f64bdaf` 及其哈希仅作为首次发布基线，不得冒充本次纠正覆盖的完成证据。
- PR #74 的自动检查通过后按用户明确授权使用管理员权限合并；`main` 与 `release/v2.3.0` 对齐到 `f931bf2097c03712d90d3aa7c30314c675e8d5e7`，GitHub Wiki 提交为 `b55a950`，Pages 运行 `33405185384` 成功。
- 最终注释 Tag 对象 `d5db7eff01faa57624b7d750f161eba982fb0d0c` 指向同一提交；原子运行 `33405585536` 成功完成源码门禁、官方文件、Android 签名与模拟器启动、Windows 两包构建与启动、Defender、10 文件审计和远端 SHA-256 回读。
- Release API 最终恰有 8 个新维护者资产并设为 Latest；工作流只删除被新集合替换的 v2.3.0 旧资产，历史 Release、历史 Tag、历史资产和本地备份未改动。

### 2026-09-05 v2.4.3 Android 版本号门禁复盘
