...
- **技术栈**：Node.js 22+、Express 5、Socket.IO 4、原生 HTML/CSS/JavaScript、Electron 41、Android Java/C++/WebView、FFmpeg/FFprobe、Gradle、PowerShell/Bash。新版本交付范围仅 Windows 与 Android；macOS 新包已停用。
...
- Release 正文第一屏必须保留产品简介、首次修改默认管理员密码提示，以及“下载文件 / 版本标识 / 最适合谁 / 一句话说明”选择表。永久使用说明与上一版本保持同等完整度；“更新公告”只写上一版本到当前版本真实发生的变化。
- 同一版本因缺陷需要重传时，只删除该版本的维护者资产并重新上传完整 10 项；不得删除历史 Release、历史 tag 或历史版本资产。重传过程中不得把资产数为 0 或不足 10 的 Release 宣称为发布完成。
- 每次发布的 5 个 SyncWatch 应用资产（Windows 4、Android 1）必须由最终 Tag 对应的修改后源码重新构建。Node.js 与 cloudflared 的 5 个官方原始分发文件不由本项目源码生成，允许从已核验的本地缓存复用，但上传前仍必须核对官方来源、版本、平台/架构、非空大小和 SHA-256；任何一项缺少证据时 Release 保持草稿。
...
- 支持平台按 Release 中真实资产说明：Windows、Android 和浏览器；macOS 仅作为历史版本资产保留，不再构建新包。GitHub Pages 仅是静态展示站，不得描述成可运行 Node.js、Socket.IO、上传、AI 或公网 Tunnel 的在线服务器。
...
- 每个正式 Release 必须有 **12 个可见文件**：10 个维护者真实资产，加 GitHub 自动生成的 `Source code (zip)` 和 `Source code (tar.gz)`。Release API 的维护者资产数必须为 10，不能用空文件、改名旧包、重复文件或占位文件凑数。
- 每次正式版本构建开始前，必须在项目根目录创建或准备唯一的 `dist/`，所有 **12 个最终交付文件都必须直接生成到该目录**：10 个经过验证的 Windows/Android 构建或官方运行资产，加与最终 Tag/提交一致的源码 ZIP 和 TAR.GZ。禁止先把正式成品生成到其他目录后再复制/汇总；构建配置、脚本和 GitHub Actions 必须以根目录 `dist/` 为最终输出路径。`dist/` 内少于或多于 12 个、存在旧版本、重复、空文件或哈希不一致时，都必须标记构建未完成。
- `dist/` 中 12 个文件并非全部都是“启动程序”：GitHub 的两个源码归档用于审阅和自行构建，其余 10 个才是应用、安装包或运行工具。文档和交付清单必须如实区分，不能把源码归档宣传成可双击启动的软件。
- 10 个资产固定分组：Windows 体验版/标准版/完整版安装 EXE/完整版便携 EXE 4 个；Android 通用 APK 1 个；Node.js Windows x64/ARM64 MSI 2 个；cloudflared Windows x64 EXE、Windows x64/x86 MSI 3 个。macOS 新包停用，历史资产保留。
- 体验版用于连接已有服务器；标准版用于基本服务器运行；完整版安装 EXE 与完整版独立便携 EXE 都必须存在，且完整包应包含承诺的 Windows 运行环境、cloudflared 和 Android 离线资源。安装程序不能替代独立 EXE，独立 EXE 也不能只做快捷方式。
- 每个资产上传前逐项核对版本、平台、架构、字节大小、SHA-256、非空状态、包内文件闭包和实际启动结果；固定 Node.js/cloudflared 文件可从已核验本地缓存复用，但不得跳过来源和哈希复核。
...
- 新版本发布顺序固定为：完成源码/配置/文档 → 本地运行和测试 → 真实构建应用并核验缓存第三方文件 → 成品契约与哈希检查 → 仅清理当前版本旧资产 → 一次性上传完整 10 项 → 更新完整 Release 正文 → 发布并设为 Latest → 验证首页、latest、Actions、资产数量和下载链接。历史 Release、历史 Tag 和历史资产永远保留。
...
- 当前版本号为 `v2.2.6`，但该版本曾发生“公开 Assets 未包含当前修正源码”的交付问题。任何旧 Tag SHA、旧 Actions run、旧资产和旧哈希都只能作为线上基线，不能作为同版本更正版完成证据。必须从最终修正 Tag 重新构建 5 个应用资产、核验 5 个官方文件或缓存，并以当前 Actions、Release API、远端哈希、启动与核心流程结果重新证明交付。
