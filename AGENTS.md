...
当前最新正式版本为 `v2.4.4`；已完成最终 Tag、原子构建、Android/Windows 启动验收、哈希回读和公开 Release；历史版本资产保持不变。

当前稳定分支为 `main`，正式源码版本为 `v2.4.4`；v2.4.4 已完成真实构建、启动、哈希和 Release 资产验收。历史 Release、历史 Tag 和历史资产保持不变。

当前工作区包含已发布的 `v2.4.4` 修复：播放时钟按媒体时长封顶、不可解码原片自动选择兼容 MP4，以及此前媒体格式、NAT/反向代理 Range 响应、移动端片库、聊天和成员 UI 回归均已完成。

当前工作区已发布 `v2.4.4`：服务端播放时钟按媒体时长封顶，客户端在原片不可解码且兼容版已就绪时自动选择 H.264/AAC 源。用户云服务器仍需手动替换程序并重启服务，GitHub Release 不会自动部署云机。
