...
当前最新正式版本为 `v2.3.1`（候选发布）；本分支正在完成全量 UI、Windows/Android 构建与远端资产验收，未完成前不得宣称已发布。

当前工作分支为 `release/v2.3.1`，源码版本为 `v2.3.1`；本轮共享性能、音源状态、主题回执、Android 启动迁移和桌面启动重试已完成本地验证，正在进行最终远端资产验收。v2.3.0 历史 Release、历史 Tag 和历史资产保持不变。
