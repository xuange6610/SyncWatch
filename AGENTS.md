...
当前最新正式版本为 `v2.4.1`（候选，已完成本地构建，待维护者确认后再进行最终 Tag、Release 和页面验收）；源码、Windows/Android 构建、远端资产和页面验收完成后方可公开，历史版本资产保持不变。

当前稳定分支为 `main`，正式源码版本为 `v2.3.7`；v2.3.7 已完成真实构建、启动、哈希和 Release 资产验收。历史 Release、历史 Tag 和历史资产保持不变。

当前工作区包含 `v2.4.1` 候选修复：发布前必须完成移动端片库、聊天、成员 UI 回归，并从最终 Tag 重新构建和核验资产。
