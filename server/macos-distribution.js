...
function releaseVersion(value) {
  const normalized = String(value || '2.2.8').trim().replace(/^v/i, '');
  return /^\d+\.\d+\.\d+(?:[-+][0-9A-Za-z.-]+)?$/.test(normalized) ? normalized : '2.2.8';
}

function macArtifactFilename(kind, architecture, format, version = '2.2.8') {
...
function createMacDistribution({
  kind, version = '2.2.8', legacyPaths = {}, configured = {}, roots = [],
