...
GitHub Release 是下载成品的地方；仓库里的 Source code 压缩包只是源码，不是双击即用的程序。v2.2.9 目前是候选版，计划提供 8 个上传文件，加 GitHub 自动生成的 2 个源码归档，共 10 个可见文件。
...
新版本不再构建或上传 macOS 安装包；历史 Release 仍按历史记录保留。标准版和完整安装版也不在 v2.2.9 下载清单中。
...
```powershell
Get-FileHash .\SyncWatch-v2.2.9-Full-Offline-Portable-x64.exe -Algorithm SHA256
```
