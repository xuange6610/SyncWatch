...
GitHub Release 是下载成品的地方；仓库里的 Source code 压缩包只是源码，不是双击即用的程序。v2.4.4 的 Windows/Android 本地构建、Windows 核心流程和候选哈希已完成；最终 Tag、远端资产和下载回读完成后才会公开为 Latest，线上 Latest 仍为 v2.4.3。
...
新版本不再构建或上传 macOS 安装包；历史 Release 仍按历史记录保留。标准版和完整安装版也不在 v2.4.4 下载清单中。
...
```powershell
Get-FileHash .\SyncWatch-v2.4.4-Full-Offline-Portable-x64.exe -Algorithm SHA256
```
...
每次发布要写清版本、文件名、平台、角色、默认端口 `20311`、默认密码修改提醒、测试结果和未提供的成品。8 个上传文件必须逐项核对非空大小、SHA-256、版本和实际来源；最终 Tag 构建阶段根目录 `dist/` 需恰好有 10 个文件（8 个上传资产加 2 个源码归档）后才能发布。
