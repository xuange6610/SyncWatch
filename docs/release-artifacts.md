...
GitHub Release 是下载成品的地方；仓库里的 Source code 压缩包只是源码，不是双击即用的程序。v2.4.3 已从最终 Tag 完成 Windows/Android 构建、启动验收、远端资产和下载回读。
...
新版本不再构建或上传 macOS 安装包；历史 Release 仍按历史记录保留。标准版和完整安装版也不在 v2.4.3 下载清单中。
...
```powershell
Get-FileHash .\SyncWatch-v2.4.3-Full-Offline-Portable-x64.exe -Algorithm SHA256
```
