...
GitHub Release 是下载成品的地方；仓库里的 Source code 压缩包只是源码，不是双击即用的程序。v2.3.1 已公开为 Latest，Release API 包含 8 个上传文件，页面另有 GitHub 自动生成的 2 个源码归档，共 10 个可见文件。
...
新版本不再构建或上传 macOS 安装包；历史 Release 仍按历史记录保留。标准版和完整安装版也不在 v2.3.1 下载清单中。
