...
v2.2.6 正在进行同版本更正发布。线上旧 Release 的 26 个维护者资产和两个源码归档只能说明旧基线，不能证明本轮修复已进入安装包；最终 Tag 构建、哈希回读和线上核对完成前，文件名不能替代对版本、平台/架构、大小、SHA-256、包闭包和启动结果的逐项核验。
...
```powershell
Get-FileHash .\SyncWatch-v2.2.6-Full-Offline-Installer-x64.exe -Algorithm SHA256
```

Linux/macOS 使用：

```bash
shasum -a 256 SyncWatch-Android-v2.2.6-universal.apk
```
...
v2.2.6 发布说明已逐项记录项目主页/Wiki/关于入口、注册申请数量撤回与删除、密码状态与安全重置、网页 URL 权威同步及跨域边界、Node 控制台等价管理入口，以及本轮全屏、简洁模式、地址隐私、迁移、队列和权限变化；没有当前构建或运行证据的内容仍须标记为未验证。
