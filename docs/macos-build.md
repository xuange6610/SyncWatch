...
当前源码候选为 v2.2.3，下面文件名是最终 Tag 构建时的预期输出，不表示已经生成或上传。最新正式下载仍为 v2.2.0；v2.2.3 的 12 个 macOS 应用资产必须在真实 macOS runner 上从最终 Tag 重新构建并逐项验证。
...
- `SyncWatch-Full-Offline-macOS-v2.2.3-x64.dmg` / `.zip`
- `SyncWatch-Full-Offline-macOS-v2.2.3-arm64.dmg` / `.zip`
...
文件名必须与以下格式一致：
v2.2.3 候选资产预期使用 `SyncWatch-Server-macOS-v2.2.3-x64.dmg`、`SyncWatch-Server-macOS-v2.2.3-arm64.zip`（客户端同理）。旧的中文本地文件名仍可被服务器兼容读取；只有来自最终 Tag、真实存在、非空且通过平台/架构/闭包/启动/SHA-256 验证的文件才允许出现在下载按钮和 `/api/public-config` 中。
