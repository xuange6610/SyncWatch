...
- `SyncWatch-Server-macOS-v2.2.6-x64.dmg` / `.zip`
- `SyncWatch-Server-macOS-v2.2.6-arm64.dmg` / `.zip`
- `SyncWatch-Client-macOS-v2.2.6-x64.dmg` / `.zip`
- `SyncWatch-Client-macOS-v2.2.6-arm64.dmg` / `.zip`
- `SyncWatch-Full-Offline-macOS-v2.2.6-x64.dmg` / `.zip`
- `SyncWatch-Full-Offline-macOS-v2.2.6-arm64.dmg` / `.zip`
...
文件名必须与以下格式一致：
v2.2.6 正式资产使用 `SyncWatch-Server-macOS-v2.2.6-x64.dmg`、`SyncWatch-Server-macOS-v2.2.6-arm64.zip`（客户端同理）。旧的中文本地文件名仍可被服务器兼容读取；只有来自最终 Tag、真实存在、非空且通过平台/架构/闭包/启动/SHA-256 验证的文件才允许出现在下载按钮和 `/api/public-config` 中。
