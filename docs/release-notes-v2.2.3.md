...
- 修复 Chromium 在断网恢复后已经继续播放、但没有补发 `playing/canplay` 时恢复键可能残留的问题；健康 `timeupdate` 会重建 3 秒稳定观察点，通过后再清空恢复状态，不会用单帧推进冒充恢复成功。
...
- 修正复用运行库门禁中 `node.h` 的固定 SHA-256 字符串，恢复为从已核验 artifact 得到的 64 位值；此前多写的字符会被 `sha256sum` 判为格式错误。
...
- 媒体恢复专项回归增加 Chromium 缺失 `playing/canplay` 的事件序列，并由真实 Electron 烟测执行 45 秒断网、Socket.IO 重认证、Range 恢复、继续播放和恢复状态清空。
- UI 文案浏览器验收改为有界等待 Chrome 调试目标，处理冷启动连接拒绝和空目标状态；Chrome 提前退出时输出退出码与 stderr 尾部，并使用磁盘临时目录规避 CI 共享内存不足，避免把浏览器启动波动误报成产品文案失败。
