...
- 客户端前台每 4 秒最多发起一个探测；连续 3 次延迟超过 500 毫秒或超时才进入 `unstable`，连续 2 次健康样本后恢复。页面或 Android WebView 进入后台后不启动新探测，后台返回的旧 ACK 被丢弃，回到前台后重置迟滞状态并立即重新测量。真正的本机 socket 断开继续明确显示“连接中断”，不会被迟滞逻辑掩盖。
- 探测使用递增 sequence、连接 epoch 和单一在途请求，服务端忽略乱序结果，质量遥测使用 volatile 发送，避免旧结果在重连或切房后覆盖新连接状态。
- 视频缓冲和控制通道质量保持两条独立路径：媒体连续 12 秒无播放时间/缓冲增长时进入最多 5 次渐进恢复；设备或 Socket 离线时不消耗重试预算，重连后继续，原画恢复用尽且已有流畅版时再自动降级。超过 32 MiB 的媒体使用 8 MiB 有界开放 Range，减少高延迟链路的续段频率，同时继续在拖动中止时关闭旧流，活动传输不应阻塞 Socket.IO 探测。此修复减少误报和周期性停顿，不承诺消除家庭上行、Tunnel、VPN/TUN 或运营商链路的真实波动。
- Quick Tunnel 的默认传输协议改为 `auto`，优先让 cloudflared 自动协商 QUIC，保留 HTTP/2 作为回退；桌面预检会在物理 IPv4/DoH Edge 直连、HTTP/2 和继承系统代理的自动协议之间切换，并在固定小响应 `/api/tunnel-health` 验证通过前不发布临时地址。应用配置仍可通过 `/api/public-config` 检查。固定 `--protocol http2` 仅用于诊断 QUIC/UDP 被拦截的情况。
...
- EPIPE 入口防护继续在所有平台做静态审计；真实父进程断开 stdout/stderr、无错误弹窗并正常退出的 smoke 固定在 Windows runner 执行，避免 Ubuntu 无桌面系统服务时的退出假失败。
