...
适用版本：v2.4.4 同版本纠正更新；新资产完成原子覆盖前，线上旧资产仍保持可下载
文档日期：2026-08-27
...
- 默认共享清晰度为原画，用户可切换自动、流畅版和原画；如果原片容器、编码或位深明显不受浏览器支持且服务端已生成兼容版，客户端在原画偏好下也会自动选择 H.264/AAC 兼容源，避免永久缓冲。

### 4.4 同步、共享与播放

- 播放、暂停、定位、音量、缓冲状态和版本号通过 Socket.IO 同步；服务端按已知媒体时长约束权威时钟，到达末尾后自动广播暂停状态，防止长时间运行的房间把新加入客户端定位到越界时间。
