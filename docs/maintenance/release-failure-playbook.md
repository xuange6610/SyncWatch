...
### 2.11 v2.4.3 最终纠正覆盖（2026-09-05）

- PR #107 合并提交 `4d8e1946437447eaa48c4200129cbb52cd1e498f` 后，唯一注释 Tag `v2.4.3` 已移动到该提交；原子运行 `33982257885` 从最终 Tag 重新完成源码门禁、签名 Android 构建与模拟器启动、Windows 体验版/完整离线版启动、第三方资产核验、10 文件审计和远端 SHA-256 回读。
- Release `v2.4.3` 已公开为 Latest，Release API 恰有 8 个维护者资产，页面另有 2 个 GitHub 源码归档；上一轮候选应用资产已被当前 Tag 真实重建的 3 个应用资产替换，历史 Release、Tag 和资产保持不变。
- 当前线上验证只证明 GitHub Release 和仓库源码已更新；用户云服务器的 NAT 后端不会被 GitHub 自动替换，部署 v2.4.3 并重启服务后才可重新验证公网媒体 Range。
