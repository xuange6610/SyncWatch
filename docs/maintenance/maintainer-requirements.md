...
当前线上正式版本：`v2.4.2`（候选，已完成媒体格式修复与本地回归，待维护者确认后再进行最终 Tag、Release 和页面验收）。

`v2.4.2` 已完成媒体格式、服务端 Range/MIME、Windows/Android 本地构建和移动端回归；尚未创建最终 Tag、上传 Release 或进行远端页面回读，v2.4.1 及历史 Release、Tag、资产保持不变。
