...
- 当前线上正式版本：`v2.2.5`；`v2.2.6` 仍是候选，且同版本资产曾被用户确认缺少源码中的修正。旧 Tag SHA、旧 Actions run、旧 26 个资产和旧哈希只能作为线上基线，不能证明更正版安装包。完成同版本重传后必须从 GitHub API、当前 Actions、最终 Tag 和远端下载哈希重新记录证据。
- 当前仓库：`https://github.com/xuange6610/SyncWatch`。
- 当前源码候选版本：`v2.2.7`；在最终 Tag、Actions、10 个维护者资产、2 个源码归档、Latest 和远端哈希全部核对前，不得宣称正式发布完成。
