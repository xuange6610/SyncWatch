...
`v2.4.4` 已完成正式发布：服务端把权威播放时钟限制在已知媒体时长并在末尾暂停；客户端在原片容器/编码/位深不受浏览器支持且兼容版已就绪时自动选择 H.264/AAC 源。最终 Tag、Windows/Android 构建与启动、哈希、Pages/Wiki/Release 回读均已完成，线上正式版本为 `v2.4.4`。
...
当前线上正式版本：`v2.4.4`（原子发布运行 `34003224268` 已完成最终 Tag、源码门禁、Android 模拟器、Windows 两套包、Defender、10 文件审计、远端 SHA-256 回读和 Latest 发布）。
...
公网实例边界：本次回读用户提供的 NAT 转发入口时，公开配置仍返回 v2.4.3；这表明转发链路可达但后端实例尚未部署 v2.4.4。发布仓库不会自动替换用户云服务器，部署后应重新回读 `/api/public-config` 版本，并用已认证媒体请求验证 Range；不得把旧实例的回读结果当作新版本成品证据。

### 2026-09-05 v2.4.3 媒体文件流打开竞态复盘
...
- 当前纠正更新源码：`main`、`release/v2.3.0` 和最终 Tag 已对齐，共享性能、音源状态、主题回执和 Electron 启动重试已进入当前 Release。
- 当前仓库：`https://github.com/xuange6610/SyncWatch`。
- 当前源码与发布状态：`v2.3.0` 的最终注释 Tag 对象 `d5db7eff01faa57624b7d750f161eba982fb0d0c` 指向提交 `f931bf2097c03712d90d3aa7c30314c675e8d5e7`；Actions run `33405585536`、8 个维护者资产、2 个源码归档、Latest 和远端哈希均已核对完成；Windows 体验版/完整便携版、Android 模拟器和 Microsoft Defender 扫描均通过。
...
- 首次发布 Tag 之后新增的共享传输与 PCM 延迟优化、实时音源状态、主题回执和 Electron 启动重试，现已通过最终 Tag 和原子运行 `33405585536` 进入新 8 项资产；首次公开证据只作历史基线，不再代表当前 Release。
...
- 最终完成证据：PR #74 合并提交 `f931bf2097c03712d90d3aa7c30314c675e8d5e7`，Tag 对象 `d5db7eff01faa57624b7d750f161eba982fb0d0c`，原子运行 `33405585536`，Wiki 提交 `b55a950`，Pages 运行 `33405185384`。Release API 恰有 8 个新资产并为 Latest。
...
### 36. v2.3.1 原子发布首次重试复盘（2026-09-01）

- 原子运行 `33463353915` 的源码、清单和前置契约均通过；`npm run test:all` 在 `tests/v224-backend.test.js` 处失败。
- 根因是 v2.3.1 新增账号级快捷键偏好后，旧回归断言仍按没有 `shortcuts` 字段的对象做深比较；服务端返回与功能实现正确。
- 已将断言补齐 `F12`、`F2`、`L`、`Escape` 四个默认快捷键；修复后必须重新通过完整源码门禁，再只触发一次原子发布。

### 37. PR #78 管理员合并授权记录（2026-09-01）
