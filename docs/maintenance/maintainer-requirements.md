...
- 每个新正式版本严格遵守 [Release 资产清单](../release/release-manifest.md)：10 个维护者真实资产，加 GitHub 自动生成的 2 个源码归档，页面共 12 个可见文件。历史版本仍按其原清单保留。
- 当前固定分组为 Windows 4、Android 1、Node.js Windows 2、cloudflared Windows 3。macOS 新包停用；任何缺失、重复、空文件或额外旧式资产都不算完整发布。
- Windows 必须同时提供体验版、标准版、完整离线安装 EXE 和完整离线便携 EXE；安装版和独立 EXE 不能互相替代。
- 完整离线包必须包含其承诺的 Windows 服务器运行环境、cloudflared、Windows 客户端和 Android APK，并通过文件闭包与体积门禁。Android APK 必须使用项目发布签名并包含声明的 ABI。
...
5. 涉及发布时核对当前范围的 10 个 API 资产、12 个页面文件、哈希、大小、下载链接、Release 正文、latest 标记和 Actions 状态。
...
- 当前版本：`v2.2.6` 候选修正版；同版本资产曾被用户确认缺少源码中的修正。旧 Tag SHA、旧 Actions run、旧资产和旧哈希只能作为线上基线，不能证明更正版安装包。完成同版本重传后必须从 GitHub API、当前 Actions、最终 Tag 和远端下载哈希重新记录证据。
- 当前仓库：`https://github.com/xuange6610/SyncWatch`。
- 当前 Pages：`https://xuange6610.github.io/SyncWatch/`。
- v2.1.7、v2.1.8、v2.1.9、v2.2.0、v2.2.3、v2.2.4、v2.2.5 的历史 Release 保持不变。v2.2.6 只有在本轮最终 Tag、原子工作流、10 个维护者资产、2 个 GitHub 源码归档、Latest 和远端哈希全部重新核对后，才可标记同版本更正版完成。
