...
工作区当前承载 `v2.4.4` 候选修复：服务端把权威播放时钟限制在已知媒体时长并在末尾暂停；客户端在原片容器/编码/位深不受浏览器支持且兼容版已就绪时自动选择 H.264/AAC 源。Windows/Android 本地真实成品、版本/大小/SHA-256 校验和 Windows 核心流程已完成；最终 Tag、Release、Pages/Wiki 回读、Android 模拟器启动和云服务器重启尚未完成，线上正式版本仍为 `v2.4.3`。
