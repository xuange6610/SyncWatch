...
工作区当前承载 `v2.4.4` 修复：服务端把权威播放时钟限制在已知媒体时长并在末尾暂停；客户端在原片容器/编码/位深不受浏览器支持且兼容版已就绪时自动选择 H.264/AAC 源。该修复已合并到 `main`；正式发布仍必须从最终 Tag 重新完成 Windows/Android 构建、启动、哈希和 Pages/Wiki/Release 回读，线上正式版本在此之前仍为 `v2.4.3`。
