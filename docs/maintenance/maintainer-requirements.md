...
当前线上正式版本：`v2.4.1`（候选，已完成本地构建，待维护者确认后再进行最终 Tag、Release 和页面验收）。

`v2.4.1` 已完成真实 Windows/Android 本地构建、移动端回归和 8 个候选资产的本地哈希核验；尚未创建最终 Tag、上传 Release 或进行远端页面回读，v2.4.0 及历史 Release、Tag、资产保持不变。
