...
`v2.4.4` 只包含播放时钟封顶和浏览器兼容源选择修复。Windows/Android 本地成品已完成真实构建、版本/大小/SHA-256 校验，Windows 完整离线包已完成启动/核心流程验收；最终 Tag、Android 模拟器启动、远端页面回读和 Release 仍待完成，Release 必须保持 pending。完成后仍需严格上传 8 个维护者资产，并保留两个 GitHub 源码归档，共 10 个可见文件。
