...
v2.1.7、v2.1.8、v2.1.9、v2.2.0、v2.2.3 和 v2.2.4 的 Release API 均已验证有 26 个维护者资产，加上 GitHub 自动源码归档后每个版本都是 28 个可见文件。v2.2.4 于 2026-08-27 复核为 Latest，Release API 有 26 项。后续版本只有在真实构建、哈希和 Release API 校验完成后，才能声明达到相同的 26 + 2 清单；不能因为 README 或公告列出名称就视为文件已经上传。

v2.2.4 的 17 个 SyncWatch 应用资产（Windows 4、Android 1、macOS 12）已从最终 Tag 重新构建并验证；Node.js 4 项和 cloudflared 5 项已按官方原始分发另行核对来源、版本、平台/架构、字节数和 SHA-256。两类证据来自原子工作流 run `33040168896` attempt 3，未复用、复制或改名上一版本应用包。

README、Wiki 和 Release 说明已按 Release API 恰有 26 个维护者资产、页面 28 个文件、Latest 指向 v2.2.4 和逐项远端哈希回读的证据切换为正式状态。后续版本在相同门禁全部完成前仍必须标为候选，不能提前把文件名写成已经可下载。
