...
`v2.4.4` 只包含播放时钟封顶和浏览器兼容源选择修复。当前尚未完成最终 Tag 的 Windows/Android 真实构建、启动/核心流程、哈希和远端页面验收，Release 必须保持 pending；线上 Latest 仍为 `v2.4.3`。完成后仍需严格上传 8 个维护者资产，并保留两个 GitHub 源码归档，共 10 个可见文件。
