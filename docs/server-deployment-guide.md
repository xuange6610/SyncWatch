...
适用版本：v2.2.5 候选（文档更新于 2026-08-27，正式发布状态待 Release API 与 Actions 验证）

本文面向需要把 SyncWatch同步观影 放到 Windows Server、Linux 云服务器、Docker 或内网穿透环境长期运行的用户。文中的 `vX.Y.Z` 表示实际部署版本；当前正式下载使用 v2.2.5，所有文件都应从当前 Release 的真实资产列表获取。
...
node server-standalone.js --port=5000 --open-browser
```
...
- 公网网页客户端会优先连接 WebSocket，失败时自动回退 polling；v2.2.5 正式客户端默认播放原画，反向代理或 Tunnel 不会自动改成流畅版。网络受限时可在播放器手动选择流畅版；该 H.264/AAC 低带宽兼容版本目标不高于 854×480、视频约 900 kbps、音频 96 kbps。即使源文件已经是浏览器可解码的 H.264 MP4，只要分辨率或平均码率超过预算也会生成低带宽版本。
