...
- 同步更新 Windows、Android、Docker、下载中心、Pages、仓库 Wiki、版本字段与发布清单为 v2.4.4；正式资产必须由最终 Tag 真实构建，并通过启动、哈希和远端页面验收。
...
本文件描述 v2.4.4 的正式发布范围。Windows 体验版、完整便携版和 Android APK 必须由最终 v2.4.4 Tag 对应源码真实重建，并完成启动/核心流程、版本、平台、非空大小和 SHA-256 核验；Node.js 与 cloudflared 文件必须按官方来源和固定缓存哈希核验。Release API 应达到 8 个维护者资产，页面另含 2 个 GitHub 源码归档；不得上传残缺集合或复用旧包。
