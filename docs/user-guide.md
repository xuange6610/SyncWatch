...
适用范围：v2.2.8 正式版；当前正式下载、哈希与资产列表以 [GitHub Release v2.2.8](https://github.com/xuange6610/SyncWatch/releases/tag/v2.2.8) 为准，Release API 有 10 个维护者资产，页面连同 2 个源码归档共 12 个文件。

## 直接使用

双击 Release 中的 `SyncWatch-Standard-Server-Portable-v2.2.8-x64.exe`，或选择完整安装版/完整便携版。首次启动时，Windows 防火墙如有提示，请按实际网络允许访问。程序会在 EXE 同目录创建 `SyncWatch同步观影-Data/`，账号、设置、媒体、缩略图、字幕、语音、聊天、缓存和密钥全部保存在这里；把整个程序文件夹移动到另一台电脑或云服务器时，数据会随文件夹一起移动。
...
3. 双击画面进入全屏；全屏后再次双击只呼出边看边聊和控制层，不触发播放/暂停。桌面可按 `F12` 切换观影全屏。全屏不弹普通通知、通告或对话框，只保留弹幕；退出后再处理待确认申请。
...
- 顶栏“生成 APK”会下载本 EXE 内置并经过签名校验的 `SyncWatch-Android-v2.2.6-universal.apk`。安装后输入服务器地址即可使用与网页端一致的房间、播放、聊天、上传、全屏和账户功能；普通客户端不显示服务器 LAN IP，公网分享只使用可信 HTTPS/Tunnel 地址。
...
EXE 未使用商业代码签名证书，首次运行可能出现 SmartScreen；正式公开分发建议配置代码签名证书。v2.2.6 有四个用途不同的 Windows EXE，不存在一个能代表全部成品的“最终 EXE”哈希；请在 [GitHub Release v2.2.6](https://github.com/xuange6610/SyncWatch/releases/tag/v2.2.6) 中按文件名核对各自的字节数和 SHA-256。
...
脚本先执行接口、硬化、媒体、Electron、多窗口同步、公网隧道和成品契约，再使用现有发布密钥构建并验证 Android v2.2.6 APK。Windows 与 Android 正式构建产物必须由最终 Tag 源码直接生成到根目录 `dist/`；缺少真实 macOS ZIP 时完整离线 EXE 必须停止构建，不会用空文件、旧包或改名文件补齐。
