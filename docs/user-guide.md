...
适用范围：v2.3.1 正式版；当前下载以 [GitHub Release v2.3.1](https://github.com/xuange6610/SyncWatch/releases/tag/v2.3.1) 为准。Release API 已核对 8 个维护者资产，页面另有 2 个源码归档，共 10 个可见文件。

## 直接使用

双击 Release 中的 `SyncWatch-v2.3.1-Full-Offline-Portable-x64.exe`。首次启动时，Windows 防火墙如有提示，请按实际网络允许访问。程序会在 EXE 同目录创建 `SyncWatch同步观影-Data/`，账号、设置、媒体、缩略图、字幕、语音、聊天、缓存和密钥全部保存在这里；把整个程序文件夹移动到另一台电脑或云服务器时，数据会随文件夹一起移动。
...
- v2.3.1 纠正更新的桌面共享默认请求电脑原生分辨率、设备最高刷新率、极致画质和系统音频，开始后会显示系统实际授予的分辨率、FPS 和音频状态。观看端优先使用 WebRTC；连接失败或断开才回退到有界画面/音频帧。房间状态条显示共享者、媒体标题和进程名，停止后立即清空；手机或网页被自动播放策略拦截时点击“开启声音”。Android 10 及以上客户端仍使用系统 MediaProjection 原生投屏。浏览器、操作系统或受保护窗口可能降低帧率、拒绝声音或只允许共享画面，无法保证固定 240 FPS 或零延迟。
...
- 顶栏“下载 APK”会下载本 EXE 内置并经过签名校验的 `SyncWatch-Android-v2.3.1-universal.apk`。安装后输入服务器地址即可使用与网页端一致的房间、播放、聊天、上传、全屏和账户功能；普通客户端不显示服务器 LAN IP，公网分享只使用可信 HTTPS/Tunnel 地址。
...
EXE 未使用商业代码签名证书，首次运行仍可能出现 SmartScreen；在取得可信 Authenticode 证书前不能保证所有杀毒软件不误报。v2.3.1 有体验版和完整便携版两个用途不同的 Windows EXE，请在 [GitHub Releases](https://github.com/xuange6610/SyncWatch/releases) 中按文件名核对各自的字节数和 SHA-256。
...
脚本先执行接口、硬化、媒体、Electron、多窗口同步、公网隧道和成品契约，再使用现有发布密钥构建并验证 Android v2.3.1 APK。Windows 与 Android 正式构建产物必须由最终 Tag 源码直接生成到根目录 `dist/`，不会用空文件、旧包或改名文件补齐。
