...
- 家庭与公司网络对照：公司网络在同一源码和隔离数据目录下完整通过 HTTPS、WebSocket/Polling、首尾 Range、Chromium 播放、共享与 40 次拖动，心跳中位约 154→156 ms；同一公司电脑启用了 FlClash TUN/Fake-IP，重复压力轮仍偶发 Polling、TLS reset 或 Range 超时。最终单项复跑走 HTTP/2 IPv4 直连并以中位 156→157 ms、0 个边缘错误通过。结论是链路通常可用但临时 Tunnel/TUN 仍可能瞬时抖动，新加的 12 秒无进度有限恢复负责避免客户端永久停帧，不把第三方临时隧道描述成固定 SLA。
- Playwright 真实视口：`1440×900` 桌面、`390×844` 手机、`844×390` 横屏与 Android WebView 模拟；检查登录、管理、观影、聊天、资料卡、AI/API、语音折叠和全屏双击。
- Android 源码闭包和三种 ABI 契约已通过；此前候选 APK 的 v1/v2/v3 签名及 Android 15 模拟器启动通过，但本轮源码冻结后的最终 APK 仍需重新构建、解包并复核，完成前不把旧候选包列为正式资产。
