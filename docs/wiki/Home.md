...
- [v2.2.6 更新公告（待同版本更正发布）](37-v2.2.6更新公告) - 全屏锁定、F2 边看边聊、简洁模式和画中画修复；旧 Release 的 26+2 文件只代表线上旧基线，待最终构建与远端哈希重新核对。
...
应用页面顶栏“关于”和桌面服务器“帮助”菜单使用上面的同一项目主页与 Wiki 地址。v2.2.6 同时补齐注册名额申请的数量撤回/删除、账户密码状态与安全重置、服务器权威网页 URL 恢复及跨域边界；纯 Node.js 控制台用 `--help`、`--open-browser` 和 `服务器运行信息.txt` 提供等价管理入口，不会伪装成带 Electron 菜单的桌面程序。
