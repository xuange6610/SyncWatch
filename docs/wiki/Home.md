...
和朋友、家人、情侣远程一起看电影。SyncWatch 是开源、自托管的 Watch Party / 同步观影系统：房主启动自己的服务器，成员通过 Windows、Android 或浏览器加入同一房间，播放、暂停、拖动进度和倍速会保持同步；macOS 仅保留历史资产。
...
1. [新手快速开始](01-新手快速开始) - 下载 v2.2.7、双击启动、首次登录、修改密码、建房和邀请成员。
...
- [v2.2.7 更新公告](37-v2.2.7更新公告) - 全屏锁定、F2 边看边聊、简洁模式、加入提示、主题双击入口和 Windows/Android 正式资产。
...
应用页面顶栏“关于”和桌面服务器“帮助”菜单使用上面的同一项目主页与 Wiki 地址。v2.2.7 同时补齐注册名额申请的数量撤回/删除、账户密码状态与安全重置、服务器权威网页 URL 恢复及跨域边界；纯 Node.js 控制台用 `--help`、`--open-browser` 和 `服务器运行信息.txt` 提供等价管理入口，不会伪装成带 Electron 菜单的桌面程序。
