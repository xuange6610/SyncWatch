...
| Windows 安装完整版 | `SyncWatch-v版本-Full-Offline-Installer-x64.exe` | 希望正常安装并给各平台成员分发客户端的房主 | 安装目录、快捷方式、卸载入口、完整 Windows 服务端和内嵌的 Windows/Android/macOS 下载文件 | 不需要另装 Node.js/cloudflared；文件超过 1 GB；首次账号密码登录后改密只填写新密码和确认密码 |
| Windows 独立 EXE 完整版 | `SyncWatch-v版本-Full-Offline-Portable-x64.exe` | 不想安装但需要完整离线下载中心的房主 | 直接双击运行完整 Windows 服务端，内嵌内容与安装完整版一致 | 文件超过 1 GB；运行目录必须有写入权限；首次账号密码登录改密不重复要求当前密码 |
...
cloudflared 与 Node.js 的区别、官方地址、安装步骤和命令示例见 [cloudflared 与 Node.js 安装使用教程](23-运行环境完整教程)。
