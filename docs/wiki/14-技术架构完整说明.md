...
- 注册申请带申请账号数量，提交设备可按数量部分撤回或全部撤回；内置 `admin` 可单条或批量删除申请记录。账户管理 API 只下发密码状态、是否待修改/过期和更新时间，不下发明文或密码哈希；安全重置会撤销旧会话。
...
- 房主可申请复制另一房间的配置、权限、队列和媒体，来源房主同意后建立独立副本；内置 `admin` 可二次确认后覆盖迁移到既有目标，目标 ID/所有者保留，失败按事务回滚。
...
- `admin.uiCopy` 保存固定白名单文案；公开配置只返回归一化纯文本，服务端拒绝未知 key、HTML、脚本、选择器和 DOM 路径，并通过 `ui-copy-state` 向在线客户端广播更新。
...
- 只有房主或被授予控制权的成员可以更新阅读位置；切换/删除文本会重置状态。客户端必须忽略旧 revision，并优先用 Range 找到 characterOffset 的行首后按本地几何滚动，窗口 resize 后重新校准；这样所有成员定位同一文件和同一字符锚点，换行差异不等于同步失败。
- 屏幕共享支持 Electron 桌面捕获、浏览器能力和 Android MediaProjection。v2.3.0 纠正更新优先为每名观看者建立 WebRTC 音视频连接，成功后停止向该观看者转发 JPEG；进入 `disconnected` 时立即恢复最高 720p/20 FPS 的有界 Socket.IO 兜底，持续断开后重建 Peer。桌面默认请求原生分辨率、检测到的设备刷新率（上限 240 FPS）、极致画质和系统音频，界面显示系统实际授予值。浏览器音频帧兜底采用 48 kHz、1024 帧、Int16 PCM 并兼容旧 Float32；同一会话瞬断迁移 `audioShare.socketId`，停止、断线超时或切房时原子清空。共享 offer/answer 按角色方向校验并限流。
- 网页共享由服务端校验并保存 URL、标题和 revision，晚加入/重连按权威状态恢复；每端在 sandbox iframe 中独立加载。跨域、Cookie、登录态、地域、CSP/X-Frame-Options 和第三方脚本状态不由 SyncWatch 绕过，需要同画面和同一操作者时改用浏览器标签页/窗口实时共享。
...
- Web“关于”和 Electron“帮助”菜单统一使用 GitHub 项目主页与 Wiki canonical 地址，并显示作者、许可证和版本边界。
- 纯 Node.js 服务端没有 Electron 顶部菜单；`--help` 给出端口、可信代理和打开浏览器参数，`--open-browser` 在就绪后打开私密管理 URL。启动摘要与 `服务器运行信息.txt` 提供管理 URL、配置/数据目录；浏览器使用 F5、F11、Ctrl+0 完成刷新、全屏和缩放。
...
| 公网入口 | Cloudflare `cloudflared` | 当前 Release 提供 Windows 对应二进制；Linux 可按官方教程安装 | 本机 `127.0.0.1:20311` 到 Cloudflare Edge 的 HTTPS/QUIC/HTTP2 隧道 |
...
该脚本会先运行完整回归，再构建并验证签名 APK、Windows 完整便携服务器和体验客户端；所有正式成品直接写入根目录 `dist/`。任一应用成品缺失或门禁失败都会停止发布。
...
| `tests/ui-copy.test.js` | 文案权限、白名单、纯文本安全、广播、导入导出和持久化。 |
