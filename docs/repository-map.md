...
5. 想重新打包：Windows 使用 `build-windows.ps1`，Android 使用 `mobile/build-apk.ps1`，独立服务端使用 `build-server-package.ps1`。
...
| `electron-client.js` | Windows 独立客户端主进程 |
| `electron-main-preload.js` | 服务器桌面端渲染进程的安全 IPC 接口 |
| `electron-pink.js` | Windows 服务器桌面端主进程 |
...
| `start-server.cmd` / `start-server.ps1` / `start-server.sh` | Windows CMD、PowerShell、Linux 三种启动入口 |
