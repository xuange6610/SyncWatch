...
| Release、文件地图和贡献 | [v2.4.3 发布说明](release-notes-v2.4.3.md)、[发布文件说明](release-artifacts.md)、[仓库文件地图](repository-map.md)、[参与贡献](contributing.html)、[CONTRIBUTING.md](../CONTRIBUTING.md) |
...
- v2.4.3 已完成媒体格式与源码回归、Windows/Android 真实成品和启动验收，并已上传 GitHub Release；历史 Release 保持不变。
- GitHub Pages 只能提供静态 HTML/CSS/JavaScript，不能执行 Node.js、Socket.IO、文件上传、AI 中转或 Cloudflare Tunnel。
- v2.4.3 新构建仅提供 Windows 与 Android；不再构建或上传 macOS 新包。
...
- 视频上传入口覆盖常见容器与编码器扩展名；桌面端为非浏览器原生编码生成兼容 MP4，公网 Range 请求保留真实视频 MIME。
