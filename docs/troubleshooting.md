...
v2.2.2 对超过 32 MiB 的影片使用 8 MiB 有界开放 Range，并在 12 秒无进展后最多渐进恢复 5 次；设备或 Socket 离线期间不消耗次数，恢复后继续当前影片，原画最终失败且已有流畅版时才自动降级。切到后台时控制通道探测暂停，回到前台后重新测量，所以后台节流造成的旧延迟不应再触发“网络波动”。如果 5 次恢复后仍停止，说明当前媒体链路持续不可用，应按上面步骤检查实际带宽和代理，而不是继续无限重载。
