...
v2.3.0 纠正更新会立即显示启动等待窗口，只有本机服务已监听、主页面加载成功并收到窗口就绪事件后才关闭；回环页面短暂未就绪时会自动进行有界重试。若等待窗口最终报告失败，先确认没有另一个 SyncWatch 实例占用数据目录或端口，再检查安全软件是否拦截 `127.0.0.1` 回环连接。不要在等待过程中连续双击启动多个实例。
...
v2.3.0 纠正更新默认请求原生分辨率和设备最高刷新率，并优先使用 WebRTC；只有未建立连接或连接断开时才使用最高 720p/20 FPS 的 Socket.IO 兜底。共享设置显示的是请求策略，开始后的状态才是系统实际授予值。Windows、浏览器、显卡编码器、被共享窗口和观看设备都可能把 240 Hz 请求降到较低帧率；公网端到端传输也不可能保证物理零延迟。先确认观看端已进入 WebRTC、系统实际授予值正常，再检查 CPU/GPU、服务器上行、丢包和观看端解码能力。
