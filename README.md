...
v2.2.1 源码候选把顶栏网络状态与视频缓冲分开判断：每 4 秒最多进行一次 Socket.IO 控制通道探测，单次高延迟、视频 `waiting/stalled` 或成员短暂重连不会单独触发“网络波动”或直接改变当前状态；连续 3 次延迟超过 500 毫秒或超时才显示“网络波动”，连续 2 次健康样本后恢复。真正的本机连接断开仍显示“连接中断”。这能减少播放一段时间后由偶发探测尖峰产生的误报，但不会掩盖真实弱网或消除公网链路波动。
