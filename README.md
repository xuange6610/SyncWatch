...
> 当前状态：v2.3.3 正在完成全量页面/客户端回归和正式构建，发布前保持 pending；本版本只提供 Windows、Android 和 Windows 上需要的辅助工具。许可证：[Apache-2.0](LICENSE) · 作者：xuan

当前正式发布：[v2.3.1](https://github.com/xuange6610/SyncWatch/releases/tag/v2.3.1)；v2.3.3 已完成源码回归，待 GitHub 远端资产验收后再切换 Latest；[v2.3.3 发布说明](docs/release-notes-v2.3.3.md)记录本轮修复范围。
...
| 下载文件 | 运行角色 | 适合谁 | 作用 |
| --- | --- | --- | --- |
| [`SyncWatch-Experience-Client-Portable-v2.3.3-x64.exe`](https://github.com/xuange6610/SyncWatch/releases/download/v2.3.3/SyncWatch-Experience-Client-Portable-v2.3.3-x64.exe) | **客户端** | 体验版；普通成员 | 输入已有服务器地址加入房间，不启动服务端 |
| [`SyncWatch-v2.3.3-Full-Offline-Portable-x64.exe`](https://github.com/xuange6610/SyncWatch/releases/download/v2.3.3/SyncWatch-v2.3.3-Full-Offline-Portable-x64.exe) | **服务器** | 完整便携版；房主 | 无需安装，下载后直接双击开房 |
| [`cloudflared-windows-x64-installer.msi`](https://github.com/xuange6610/SyncWatch/releases/download/v2.3.3/cloudflared-windows-x64-installer.msi) / [`cloudflared-windows-x86-installer.msi`](https://github.com/xuange6610/SyncWatch/releases/download/v2.3.3/cloudflared-windows-x86-installer.msi) | **服务器工具** | 公网访问工具 | 双击 MSI 安装；安装后在终端执行 `cloudflared --version`，再按教程创建 Tunnel；[Cloudflare 官网](https://developers.cloudflare.com/cloudflare-one/connections/connect-networks/) · [官方 Release 下载](https://github.com/cloudflare/cloudflared/releases/latest) |
| [`node-v24.19.0-x64.msi`](https://github.com/xuange6610/SyncWatch/releases/download/v2.3.3/node-v24.19.0-x64.msi) / [`node-v24.19.0-arm64.msi`](https://github.com/xuange6610/SyncWatch/releases/download/v2.3.3/node-v24.19.0-arm64.msi) | **服务器环境** | 源码/独立服务器环境 | 正式 SyncWatch EXE 无需另装；源码和独立服务端安装后运行 `node --version` 验证；[Node.js 官网](https://nodejs.org/) · [官方下载](https://nodejs.org/en/download) |
| [`SyncWatch-Android-v2.3.3-universal.apk`](https://github.com/xuange6610/SyncWatch/releases/download/v2.3.3/SyncWatch-Android-v2.3.3-universal.apk) | **客户端** | Android 用户 | 加入已有房间；完整包可在受支持设备上运行手机服务器 |
...
| Source code：[ZIP](https://github.com/xuange6610/SyncWatch/archive/refs/tags/v2.3.3.zip) / [TAR.GZ](https://github.com/xuange6610/SyncWatch/archive/refs/tags/v2.3.3.tar.gz) | 开发者 | 只用于阅读、修改和自行构建，不是可双击安装包 |

## 第一次启动服务器

### 使用服务器 EXE

1. 从 Releases 下载 `SyncWatch-v2.3.3-Full-Offline-Portable-x64.exe`，双击即可开房。
...
- **共享能力**：浏览器、Electron 和 Android 屏幕共享；当前源码默认请求原生分辨率、设备最高刷新率、极致画质和系统音频，并优先使用 WebRTC，只有连接未建立或断开时才使用有界 JPEG/PCM 兜底。手机进入全屏后可选择竖屏、横屏或自动横屏，默认尝试自动横屏。Tunnel 兜底会按 ACK 延迟自适应画面尺寸、JPEG 质量与采集节奏，并只保留最新帧，降低公网卡顿；房间会显示共享者、音频标题与进程名。实际分辨率、帧率、音频捕获和端到端延迟仍由操作系统、浏览器、编码器、设备与网络共同决定。
