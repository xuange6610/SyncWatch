...
async function main() {
  ...
  const clientDownloadCandidates = [
    path.join(ROOT_DIR, 'dist', 'SyncWatch-Experience-Client-Portable-v2.2.6-x64.exe'),
    path.join(ROOT_DIR, 'SyncWatch同步观影-Client-v2.2.6.exe'),
    path.join(ROOT_DIR, 'client', 'SyncWatch同步观影-Client-v2.2.6.exe')
  ];
  ...
  const macServerDownloadPaths = macArtifactCandidates('SyncWatch-Server-macOS-v2.2.6');
  const macClientDownloadPaths = macArtifactCandidates('SyncWatch-Client-macOS-v2.2.6');
