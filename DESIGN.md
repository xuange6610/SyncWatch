...
首屏使用真实登录界面作为全宽背景，文字直接叠放其上，不增加浮动卡片。登录页桌面顶部留白控制在 12px，移动端控制在 18px；公告仍在登录卡片内流动，避免工具栏下方出现无内容的横向空带。短窗口或高 DPI 缩放时登录卡片从顶部开始并由登录页统一滚动，确保底部下载、其他登录方式和版本信息可达。数据表允许自身横向滚动，但不得让整页产生横向滚动。按钮、工具栏和截图容器使用稳定最小高度，避免内容加载时布局跳动。
