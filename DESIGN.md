...
观影工作区在 924px 以下改为单列，用“观影 / 聊天 / 片库 / 成员”四个显式模块入口取代三栏并排；模块入口属于普通文档流，随页面一起滚动，不悬浮遮住播放器或工具。片库和成员保持可点击的全宽抽屉，聊天只在选中时占用主区域。房主可从房主控制、账户菜单或“我的房间”自有房间卡片进入房间设置；房主范围的管理中心只显示“房间与上传”“成员与权限组”“聊天与记录”三个模块。更换房间弹窗的自有房间列表在标题下提供单行搜索框，按房主名、房间号和房间名实时模糊筛选；筛选计数、无结果提示和批量选择都与当前可见列表一致。540px 以下观影工具使用两列重排，触控目标不低于 44px，禁止因字段过多发生整页横向溢出。手机顶栏功能菜单使用 fixed 覆盖层，不参与主界面高度计算；选项、房间操作、设置和账号操作统一为单列手风琴列表，展开项完整显示并在覆盖层内纵向滚动。桌面同一菜单只在悬停或点击时展开：悬停离开后自动收起，点击可固定，再次点击或点击外部关闭。复选框、单选框与普通文本输入分开定义尺寸，不继承文本框最小高度。
