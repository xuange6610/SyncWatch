...
8. 屏幕共享优先为每名观看者建立 WebRTC 音视频连接；连接成功后服务端停止向该观看者转发 JPEG 帧，连接失败或断开才恢复最高 720p/20 FPS 的有界 Socket.IO 兜底。默认约束请求原生分辨率、测得的设备刷新率（上限 240 FPS）、极致画质和系统音频，但 UI 必须展示系统实际授予值，不能把请求值冒充采集结果。
