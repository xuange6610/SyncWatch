...
文本阅读状态只保存 `fileId`、归一化位置、页码、修订号和操作者，不把正文放进 Socket 消息。文本正文始终以 `text/plain`、`nosniff` 和限制性 CSP 返回，前端使用 `textContent`；扩展名、MIME、全文件二进制检测和 10MB 限制在服务端统一执行。
...
4. 上传写入 `SyncWatch同步观影-Data/uploads/`，FFprobe/FFmpeg 更新媒体索引和兼容产物；公网优先使用不高于 854×480、视频约 900 kbps、音频 96 kbps 的 H.264/AAC 流畅版。HTTP Range 负责媒体读取，Socket.IO 不承载媒体本体。
