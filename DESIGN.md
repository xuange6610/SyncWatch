...
首屏使用真实登录界面作为全宽背景，文字直接叠放其上，不增加浮动卡片。登录页桌面顶部留白控制在 12px，移动端控制在 18px；公告仍在登录卡片内流动，避免工具栏下方出现无内容的横向空带。短窗口或高 DPI 缩放时登录卡片从顶部开始并由登录页统一滚动，手机端使用自动高度的文档滚动和 `touch-action: pan-y`，确保手指可以连续查看底部下载、其他登录方式和版本信息。数据表允许自身横向滚动，但不得让整页产生横向滚动。按钮、工具栏和截图容器使用稳定最小高度，避免内容加载时布局跳动。
...
全屏有原生 Fullscreen API 和应用内 `.fullscreen-active` 两条入口，但必须呈现同一观影层级：媒体在底层，弹幕在其上，用户主动调出的边看边聊与退出/控制入口在最上层；Toast、通告、授权窗、模态框和普通状态提示全部隐藏。双击只展开边看边聊/控制层，原生视频控件发出的播放事件要经过手势消歧，不能向房间广播误触暂停。聊天卡片不得使用固定最小宽度，低高度或高缩放时操作区自身滚动，退出入口始终可达。
...
- 登录页的服务器管理快捷入口属于顶栏上下文操作，不放在认证卡片内部；按钮使用 30px 紧凑高度，在窄屏下横向滚动，并且只在未登录或管理专用会话显示。登录音乐控制条使用稳定三列布局，播放/暂停和静音使用独立图标按钮，进度与客户端音量放在可展开区；播放、静音和音量偏好按设备持久化，不改写服务器的全局音乐设置。账号总览使用独立加载状态和错误状态，网络或权限失败时明确告知下一步，不使用无限加载占位。
