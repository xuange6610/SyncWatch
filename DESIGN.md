...
中文系统字体保证 Windows 和 Android 上无需下载字体即可稳定阅读。标题使用较重字重建立操作层级，正文保持 1.7 行高，命令块使用明显更宽松的 1.9 行高。
...
首屏使用真实登录界面作为全宽背景，文字直接叠放其上，不增加浮动卡片。登录页桌面顶部留白控制在 12px，移动端控制在 18px；公告仍在登录卡片内流动，避免工具栏下方出现无内容的横向空带。短窗口或高 DPI 缩放时登录卡片从顶部开始并由登录页统一滚动，手机端使用自动高度的文档滚动和 `touch-action: pan-y`，并在窄屏后置覆盖中继续保持该约束，确保手指可以连续查看底部下载、其他登录方式和版本信息。数据表允许自身横向滚动，但不得让整页产生横向滚动。按钮、工具栏和截图容器使用稳定最小高度，避免内容加载时布局跳动。

观影工作区在 924px 以下改为单列，用“观影 / 聊天 / 片库 / 成员”四个显式模块入口取代三栏并排；模块入口属于普通文档流，随页面一起滚动，不悬浮遮住播放器或工具。片库和成员保持可点击的全宽抽屉，聊天只在选中时占用主区域。房主可从房主控制、账户菜单或“我的房间”自有房间卡片进入房间设置；房主范围的管理中心只显示“房间与上传”“成员与权限组”“聊天与记录”三个模块。540px 以下观影工具使用两列重排，触控目标不低于 44px，禁止因字段过多发生整页横向溢出。手机顶栏功能菜单使用 fixed 覆盖层，不参与主界面高度计算；390px 起使用三列，较窄视口使用可滚动两列。账号行只负责展开同一覆盖层内的双列账号菜单，两个退出操作排在首行，Android 触控高度不低于 48px。复选框、单选框与普通文本输入分开定义尺寸，不继承文本框最小高度。

全屏有原生 Fullscreen API 和应用内 `.fullscreen-active` 两条入口，但必须呈现同一观影层级：媒体在底层，弹幕在其上，用户主动调出的边看边聊与退出/控制入口在最上层；Toast、通告、授权窗、模态框和普通状态提示全部隐藏。双击只展开边看边聊/控制层，`F2` 或回车展开边看边聊并聚焦输入框，`L` 切换画面锁定；输入框正在编辑时不得劫持这些按键。原生视频控件发出的播放事件要经过手势消歧，不能向房间广播误触暂停。聊天卡片不得使用固定最小宽度，低高度或高缩放时操作区自身滚动，退出入口始终可达。

账户级简洁模式使用现有深色影院色板而不是另起主题。它隐藏片库、成员、公告、聊天和房主高级控制，只保留媒体、播放/暂停、进度、音量、全屏以及返回普通模式的入口；桌面和手机都必须能恢复。弹幕颜色/字号通过 CSS 自定义属性应用，聊天“仅聊天”过滤器只改变当前账号的显示，不改变服务端历史数据。

管理员发起的客户端模式切换必须是可撤销申请，不是远程强制修改。管理中心按指定用户、当前房间和全服务器选择目标，客户端使用统一确认弹窗显示发起者、模式和原因；全屏期间延后到退出全屏，离线用户登录后补投。用户同意后，服务端一次更新通知偏好与界面偏好并广播到该账号的其他在线设备；拒绝或管理员取消不会改变现有偏好。

“关于”是产品身份和帮助的单一入口：显示 SyncWatch同步观影、作者 `xuan`、Apache-2.0、GitHub 项目主页和 Wiki，不复制易过期的版本说明。Electron “帮助”菜单使用同一组 canonical URL；纯 Node.js 控制台不伪造桌面菜单，而是以 `--help`、`--open-browser`、启动摘要和 `服务器运行信息.txt` 提供等价管理入口。

Electron 首次启动先绘制与银幕主题一致的深色金色启动页，再延迟加载服务端重模块，避免慢机器在没有任何反馈的情况下长时间停住。“运行信息”使用应用内无边框深色窗口展示版本、局域网地址和数据目录，不回退到 Windows 默认白色消息框。

注册申请控件必须同时显示申请总数量与剩余数量；部分撤回要求用户输入 1 到当前剩余数，全部撤回后状态不可再次处理。删除申请属于内置 `admin` 的管理动作，支持单项和选中项批量删除并使用危险确认。账户管理卡只呈现“未设置 / 已安全设置 / 待修改 / 已过期 / 最近更新”状态；界面不得渲染密码、哈希或可逆凭据。安全重置明确提示会重置到服务器配置的默认密码、撤销现有会话，并让用户在后续流程修改。

“同步网址”与“实时共享网页画面”是两个视觉和技术模式。同步网址保存服务器权威 URL/revision，各端在带 sandbox 的 iframe 中独立加载；地址栏必须说明跨域、Cookie、登录态、地区、CSP 和禁止嵌入会导致差异，不能承诺跨域点击或滚动互控。实时共享使用浏览器标签页/窗口捕获，适用于需要同画面和主持人操作演示的场景。

桌面服务器继续在 `0.0.0.0` 监听以保持本机、局域网和 Tunnel 入口，手动网卡选择通过请求的本机目标 IPv4 限定局域网接入，不改变 Electron 主窗必需的 `127.0.0.1` 回环通路。配置网卡不存在时回退至自动选择；公网分享地址优先使用当前 HTTPS/Tunnel Origin，其次使用已配置的公网根地址。只有 `window.SyncWatchPlatform.serverApp === true` 的真实服务器应用显示 LAN 状态；普通 Electron 客户端、Android 和浏览器没有可信公网地址时必须空置，不能回退内网 IP。
...
- 登录页的服务器管理快捷入口位于登录操作下方，按钮使用 30px 紧凑高度，在窄屏下自动换行，并且只在未登录或管理专用会话显示。顶栏菜单面板保持在可视层级，菜单内的每个按钮始终显示图标和功能名称，即使顶栏处于精简模式；手机端展开菜单后面板跨列静态排列，避免只显示图标、被滚动层裁剪或遮挡登录内容。服务器端局域网状态支持双击打开详情。登录音乐控制条使用稳定三列布局，播放/暂停和静音使用独立图标按钮，静音按钮默认只显示一个状态图标，进度与客户端音量放在可展开区；播放、静音和音量偏好按设备持久化，不改写服务器的全局音乐设置。音乐编辑器把曲目列表作为完整目标状态，当前曲目的名称、地址和 ID 必须来自同一条曲目；选中文件会清除旧 HTTPS 输入，保存外链会自动创建列表曲目。展开播放条后可选择曲目、上一首/下一首，并使用列表循环、随机、单曲循环或播放一次模式。账号总览使用独立加载状态和错误状态，网络或权限失败时明确告知下一步，不使用无限加载占位。
...
- 运行库复用不等于复用 SyncWatch 应用包：Android APK、Windows 体验版和 Windows 完整便携版仍必须从最终 Tag 的源码重新构建，并完成签名（适用时）、闭包、启动和核心流程验证。
- 原子发布器只在 8 个维护者资产、两份源码归档、SHA-256 清单和远端下载回读全部一致后，才把 Release 从草稿切换为公开并设为 Latest。

### 待确认边界

### v2.2.8 交互补充

- 网页共享是“地址同步”与“实时画面共享”两条明确路径：地址模式使用各端 iframe 独立加载，标签页/窗口共享使用现有屏幕共享链路；切换到地址模式会广播清空播放状态，避免旧视频残留。
- 视频管理行展示服务器生成的缩略图，并提供批量随机封面操作。封面截取时间位于视频中段随机窗口，失败时保留原封面并返回逐项结果。
- 观影记录中的 `progress` 作为账户级恢复点；选择影片后由客户端一次性提示继续观看，确认后通过权威播放命令同步定位，避免自动静默跳转造成误操作。

### v2.2.8 界面与安全补充

- 房间工具中的倍速控件遵循“房主优先”规则：房主无论成员权限组配置如何都可调整共享倍速；普通成员仍由 `control` 权限决定。
- “跳过片头和片尾设置”作为独立 `skipSettings` 能力进入成员权限与权限组编辑器，设置模态只在房主、超级管理员或获授权成员可写时启用，服务端再次校验后广播房间状态。

- 服务器启动设置采用可滚动主内容区与底部吸附操作栏，适配短屏和系统缩放。
- 登录状态提示使用可换行的全宽操作按钮，避免多设备申请文案被挤压。
- Electron 服务端登录页在短窗口与高 DPI 下从顶部布局，外层页面负责滚动，视觉立体方块固定合理高度并允许溢出可见。
- 管理中心在标题旁显示当前主题编号与名称，和房间顶部主题状态保持一致。
- 账号密码管理不提供历史明文读取；管理员通过“设置新密码”完成显式重置，服务端立即撤销旧会话并只保存 PBKDF2 哈希。

- v2.2.9 起不再生成 macOS 新包；Windows 与 Android 的窗口、权限、签名和核心流程必须分别在对应平台或隔离模拟器中验证。
