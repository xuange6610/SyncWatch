...
观影工作区在 924px 以下改为单列，用“观影 / 聊天 / 片库 / 成员”四个显式模块入口取代三栏并排；模块入口属于普通文档流，随页面一起滚动，不悬浮遮住播放器或工具。片库和成员保持可点击的全宽抽屉，聊天只在选中时占用主区域。房主可从房主控制、账户菜单或“我的房间”自有房间卡片进入房间设置；房主范围的管理中心只显示“房间与上传”“成员与权限组”“聊天与记录”三个模块。更换房间弹窗的自有房间列表在标题下提供单行搜索框，按房主名、房间号和房间名实时模糊筛选；筛选计数、无结果提示和批量选择都与当前可见列表一致。540px 以下观影工具使用两列重排，触控目标不低于 44px，禁止因字段过多发生整页横向溢出；手机端直接移除实时语音栏、悬浮播放和重复播放节点，全屏与全屏自动锁屏固定第一行，-10/+10、画质/倍速、共享屏幕/共享电脑音源分别同行。微信内置浏览器的影片上传入口显示选择器可能限制五分钟视频的提示，并引导用户改用系统浏览器或客户端，不能把微信限制误写成服务端限制。手机顶栏功能菜单使用 fixed 覆盖层，不参与主界面高度计算；选项、房间操作、设置和账号操作统一为单列手风琴列表，展开项完整显示并在覆盖层内纵向滚动。桌面同一菜单只在悬停或点击时展开：悬停离开后自动收起，点击可固定，再次点击或点击外部关闭。复选框、单选框与普通文本输入分开定义尺寸，不继承文本框最小高度。
...
Electron 首次启动先绘制与银幕主题一致的深色金色启动页，再延迟加载服务端重模块，避免慢机器在没有任何反馈的情况下长时间停住。启动页保持到本机服务已监听、主窗口页面加载成功且收到 `ready-to-show`；本机回环页面短暂未就绪时使用有界指数退避重试，失败后显示可操作的启动错误。“运行信息”使用应用内无边框深色窗口展示版本、局域网地址和数据目录，并为两项分别提供复制与打开操作，不回退到 Windows 默认白色消息框。
...
“同步网页观影”默认保存服务器权威 URL/revision，并引导房主打开目标网页后选择浏览器标签页/窗口捕获；房间成员通过现有屏幕共享链路接收同一实时画面。浏览器 DRM、系统保护窗口、CSP、Cookie 和录屏授权限制必须如实提示。兼容预览 iframe 仅用于本机独立查看，不能承诺跨域点击、滚动或播放互控。
...
媒体网络恢复复用单一有限重试状态。`MediaError(code=2)` 与连续 12 秒没有播放时间/缓冲增长的 `waiting/stalled` 都进入同一条最多 5 次、500 毫秒到 12 秒渐进退避的恢复路径；设备或 Socket 离线时以 1 秒间隔等待但不消耗重试预算，重连后唤醒恢复，原画次数用尽且已有流畅版时再降级。恢复后要求至少 3 秒连续健康播放才清空次数；如果 Chromium 在网络切换后继续发送 `timeupdate` 却没有补发 `playing/canplay`，健康进度事件会重建同一个稳定观察点，防止恢复键永久遗留。正常缓冲增长、换片和清空画面会取消计时，避免弱网半开连接永久停帧，也避免健康播放被循环重载。超过 32 MiB 的媒体把开放 Range 限制为 8 MiB，减少高 RTT 链路的续段频率，同时不恢复会在拖动后长期占用传输的无限响应。服务端会按已知媒体时长封顶权威播放时钟，到达末尾后自动广播暂停，避免房间长时间播放后新加入成员跳到越界位置。

v2.4.4 的上传选择器和服务端分类覆盖常见容器与编码器扩展名，包括 MP4、AVI、MOV、MKV、FLV、WMV、RM/RMVB、3GP、M4V、ASF/ASX、DAT、VOB、TS、WebM、MPEG/MPG、DivX/XviD、ProRes、AV1、H.264/H.265 与 VP9。扩展名只决定输入分类，FFprobe 的实际探测结果仍决定是否能生成缩略图或兼容 MP4；服务端向公网返回原始 `video/*` MIME 与 Range 头，避免浏览器在解码前因 `application/octet-stream` 被拒绝。
...
4. 上传写入 `SyncWatch同步观影-Data/uploads/`，FFprobe/FFmpeg 更新媒体索引和兼容产物；客户端默认使用原画，流畅版是网络受限时可手动选择的低带宽 H.264/AAC 兼容版本（目标不高于 854×480、视频约 900 kbps、音频 96 kbps），当原片容器、编码或位深明显不受浏览器支持且兼容版已就绪时，原画偏好也会自动切换到兼容源。HTTP Range 负责媒体读取，Socket.IO 不承载媒体本体；服务端先等待媒体文件描述符成功打开再启动响应管线，挂载/权限/句柄错误保留为 HTTP 错误，不把失败响应重置成不可诊断的 TCP 断开。
...
8. 屏幕共享优先为每名观看者建立 WebRTC 音视频连接；连接成功后服务端停止向该观看者转发 JPEG 帧，连接失败或断开才恢复有界 Socket.IO 兜底。轮询/Tunnel 兜底根据最近 ACK 延迟在 960×540–1440×810 之间自适应尺寸、JPEG 质量和采集节奏，并按观看者只保留一帧在途与一帧最新待发，避免高 RTT 时队头阻塞；默认约束仍请求原生分辨率、测得的设备刷新率（上限 240 FPS）、极致画质和系统音频，但 UI 必须展示系统实际授予值，不能把请求值冒充采集结果。
9. 浏览器音频兜底使用 48 kHz、1024 帧、Int16 PCM 与交互型音频上下文，并兼容旧 Float32 数据；抖动目标为约 40 ms。房间权威 `audioShare` 状态包含共享者、媒体标题、进程名和来源类型，不包含本地路径；同一会话瞬断恢复时迁移到新 socket 并重建 Peer，停止、断线超时或房间切换时原子清空并广播。自动播放被阻止时保留明确的“开启声音”操作，状态条按实际 seek 控件高度避让。
10. 共享信令按角色锁定方向：offer 只能从共享者发给观看者，answer 只能从观看者回给共享者，ICE candidate 限制在同房间同一共享链路并统一限流。画面 Peer 进入 `disconnected` 时立即恢复 JPEG 兜底，持续断开后有界重建，不能无限冻结在无帧状态。
...
管理中心上传限制使用数值输入与 KB、MB、GB、TB 单位选择，文件下限和上限并列呈现并在保存前校验范围。顶部房间操作菜单中的“我的房间”和“房间设置”遵循当前房间管理权限；在线成员指标保持紧凑，双击后使用统一应用对话框编辑人数上限。
...
- v2.3.1 起不再生成 macOS 新包；Windows 与 Android 的窗口、权限、签名和核心流程必须分别在对应平台或隔离模拟器中验证。
